...
uploaded_file = st.file_uploader("Choose an image...", type=["jpg", "png", "nii"])

if uploaded_file is not None:
    if uploaded_file.name.endswith('.nii'):
        # Чтение содержимого файла в байтовый поток
        file_bytes = uploaded_file.read()
        nii_image = nib.Nifti1Image.from_bytes(file_bytes)
        nii_data = nii_image.get_fdata()
        slice_index = st.slider("Select slice", 0, nii_data.shape[2] - 1, nii_data.shape[2] // 2)
        image = nii_data[:, :, slice_index]
